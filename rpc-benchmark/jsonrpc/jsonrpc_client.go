...
var server_host = flag.String("s", "127.0.0.1:8975", "server ip and port")
var server_debugAddr = flag.String("d", "127.0.0.1:9986", "server ip and port")

func main() {
	flag.Parse()

	go func() {
		log.Println(http.ListenAndServe(*server_debugAddr, nil))
	}()
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			client, err := jsonrpc.Dial("tcp", *server_host)
			if err != nil {
...
func prepareArgs() *BenchmarkMessage {
	b := true
	var i int = 100000
	var i_32 int32 = 100000
	var i_64 int64 = 100000
	...
	for k := 0; k < num; k++ {
		...
		if field.Type().Kind() == reflect.Ptr {
			...
			case reflect.Int32:
				field.Set(reflect.ValueOf(&i_32))
			case reflect.Int64:
				field.Set(reflect.ValueOf(&i_64))
